<template>
  <v-app-bar
      app
      clipped-right
      elevate-on-scroll
      class="turf-page-header"
  >
    <v-app-bar-nav-icon @click="changeNavDrawerShow"></v-app-bar-nav-icon>

    <v-toolbar-title v-if="showInSize" @click="goToMainPage" class="turf-page-title-icon">Node Turf 文档</v-toolbar-title>

    <v-spacer></v-spacer>

    <div class="turf-page-header-search">
      <v-text-field
          class="mx-lg-auto"
          label="搜索"
          placeholder="搜索（按 回车）"
          outlined
          height="64"
          clearable
          :value="searchValue"
          @blur="handleBlur"
          @keydown="handleKeyDown"
      >
        <template slot="prepend-inner">
          <svg viewBox="0 0 24 24" role="img" aria-hidden="true"
               class="v-icon__svg">
            <path
                d="M9.5,3A6.5,6.5 0 0,1 16,9.5C16,11.11 15.41,12.59 14.44,13.73L14.71,14H15.5L20.5,19L19,20.5L14,15.5V14.71L13.73,14.44C12.59,15.41 11.11,16 9.5,16A6.5,6.5 0 0,1 3,9.5A6.5,6.5 0 0,1 9.5,3M9.5,5C7,5 5,7 5,9.5C5,12 7,14 9.5,14C12,14 14,12 14,9.5C14,7 12,5 9.5,5Z"></path>
          </svg>
        </template>
      </v-text-field>
    </div>

    <div class="turf-page-header-division"></div>

    <v-btn text small @click="goToGitHub">
      GitHub
      <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" x="5px" y="2px" viewBox="0 0 100 100" width="12"
           height="12" class="icon" style="margin-bottom: 2px;margin-left: 4px;">
        <path fill="currentColor"
              d="M18.8,85.1h56l0,0c2.2,0,4-1.8,4-4v-32h-8v28h-48v-48h28v-8h-32l0,0c-2.2,0-4,1.8-4,4v56C14.8,83.3,16.6,85.1,18.8,85.1z"></path>
        <polygon fill="currentColor"
                 points="45.7,48.7 51.3,54.3 77.2,28.5 77.2,37.2 85.2,37.2 85.2,14.9 62.8,14.9 62.8,22.9 71.5,22.9"></polygon>
      </svg>
    </v-btn>

    <div class="turf-page-header-division"></div>
    <v-btn
        class="mx-2 turf-page-header-btn"
        fab
        small
        title="设置"
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" role="img" aria-hidden="true"
           class="v-icon__svg">
        <path
            d="M12,8A4,4 0 0,1 16,12A4,4 0 0,1 12,16A4,4 0 0,1 8,12A4,4 0 0,1 12,8M12,10A2,2 0 0,0 10,12A2,2 0 0,0 12,14A2,2 0 0,0 14,12A2,2 0 0,0 12,10M10,22C9.75,22 9.54,21.82 9.5,21.58L9.13,18.93C8.5,18.68 7.96,18.34 7.44,17.94L4.95,18.95C4.73,19.03 4.46,18.95 4.34,18.73L2.34,15.27C2.21,15.05 2.27,14.78 2.46,14.63L4.57,12.97L4.5,12L4.57,11L2.46,9.37C2.27,9.22 2.21,8.95 2.34,8.73L4.34,5.27C4.46,5.05 4.73,4.96 4.95,5.05L7.44,6.05C7.96,5.66 8.5,5.32 9.13,5.07L9.5,2.42C9.54,2.18 9.75,2 10,2H14C14.25,2 14.46,2.18 14.5,2.42L14.87,5.07C15.5,5.32 16.04,5.66 16.56,6.05L19.05,5.05C19.27,4.96 19.54,5.05 19.66,5.27L21.66,8.73C21.79,8.95 21.73,9.22 21.54,9.37L19.43,11L19.5,12L19.43,13L21.54,14.63C21.73,14.78 21.79,15.05 21.66,15.27L19.66,18.73C19.54,18.95 19.27,19.04 19.05,18.95L16.56,17.95C16.04,18.34 15.5,18.68 14.87,18.93L14.5,21.58C14.46,21.82 14.25,22 14,22H10M11.25,4L10.88,6.61C9.68,6.86 8.62,7.5 7.85,8.39L5.44,7.35L4.69,8.65L6.8,10.2C6.4,11.37 6.4,12.64 6.8,13.8L4.68,15.36L5.43,16.66L7.86,15.62C8.63,16.5 9.68,17.14 10.87,17.38L11.24,20H12.76L13.13,17.39C14.32,17.14 15.37,16.5 16.14,15.62L18.57,16.66L19.32,15.36L17.2,13.81C17.6,12.64 17.6,11.37 17.2,10.2L19.31,8.65L18.56,7.35L16.15,8.39C15.38,7.5 14.32,6.86 13.12,6.62L12.75,4H11.25Z"></path>
      </svg>
    </v-btn>

    <v-menu
        offset-y
        color="primary"
        open-on-hover
        transition="slide-y-transition"
    >
      <template v-slot:activator="{ on, attrs }">
        <v-btn class="turf-page-header-btn" v-bind="attrs" v-on="on">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" role="img" aria-hidden="true"
               class="v-icon__svg">
            <path
                d="M12.87,15.07L10.33,12.56L10.36,12.53C12.1,10.59 13.34,8.36 14.07,6H17V4H10V2H8V4H1V6H12.17C11.5,7.92 10.44,9.75 9,11.35C8.07,10.32 7.3,9.19 6.69,8H4.69C5.42,9.63 6.42,11.17 7.67,12.56L2.58,17.58L4,19L9,14L12.11,17.11L12.87,15.07M18.5,10H16.5L12,22H14L15.12,19H19.87L21,22H23L18.5,10M15.88,17L17.5,12.67L19.12,17H15.88Z"></path>
          </svg>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" role="img" aria-hidden="true"
               class="v-icon__svg" style="font-size:14px;height:14px;width:14px;">
            <path d="M7.41,8.58L12,13.17L16.59,8.58L18,10L12,16L6,10L7.41,8.58Z"></path>
          </svg>
        </v-btn>
      </template>

      <div style="padding: 0 10px;">
        <h4 class="pop-h4">翻译</h4>
        <v-list class="pop-list">
          <v-list-item
              v-for="(item, index) in languages"
              :class="selectedLang === item?'active-pop-list':''"
              :key="index"
              @click="handleListClick(item)"
          >
            <v-list-item-title>{{ item }}</v-list-item-title>
          </v-list-item>
        </v-list>
      </div>
    </v-menu>

  </v-app-bar>
</template>

<script>
export default {
  name: "PageHeader",
  data() {
    return {
      searchValue: "",
      languages: ['简体中文', 'English'],
      selectedLang: '简体中文',
    }
  },
  computed: {
    // 尺寸控制
    showInSize() {
      switch (this.$vuetify.breakpoint.name) {
        case 'xs':
          return false;
        case 'sm':
          return false;
        case 'md':
          return false;
        default:
          return true;
      }
    }
  },
  methods: {
    // drawer按钮点击事件
    changeNavDrawerShow() {
      this.$emit('changeNavDrawerShow');
    },
    // 下拉框选中
    handleListClick(data) {
      this.selectedLang = data;
    },
    // 搜索输入框失焦事件钩子
    handleBlur(e) {
      this.search();
    },
    // 键盘按键事件钩子
    handleKeyDown(e) {
      if (e.keyCode === 13) {
        this.search();
      }
    },
    search() {
    },
    // 跳转到github
    goToGitHub() {
      window.open('https://github.com/chenli965870230/node-turf');
    },
    // 跳转到主页
    goToMainPage() {
      window.open('/', '_self')
    },
  }
}
</script>

<style scoped>

</style>
